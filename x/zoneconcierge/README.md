...
- **Verifiability of timestamps:** Babylon can provide proof that a given
...
hold even if Babylon and a PoS blockchain have a dishonest supermajority. The
formal security analysis of the Bitcoin timestamping protocol can be found in
...
agreed upon by all honest validators.
...
**Providing Proof that a Header is Checkpointed by Bitcoin.** To support use
...
developed by the Babylon team in the future.
...
blockchain will get notified of this dishonest majority attack, and can decide
...
finished, leading to a much shorter unbonding period compared to that in existing
...
2. If the header is on a fork, insert the header into the fork storage and update
   `ChainInfo`.
...
3. For each of these IBC channels:
   ...
   5. Generate the proof that the epoch is sealed, i.e., receives a BLS
      multi-signature generated by validators with >2/3 total voting power at the
...
Note that Zone Concierge provides a 1-to-all connection, where the Zone Concierge
module establishes an IBC channel with each of multiple consumer chains. Zone
Concierge will send a BTC timestamp to each of these consumer chains upon an
epoch is finalized.
